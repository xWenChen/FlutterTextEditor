...
class FileManager {
  ...
  /// 删除排队：手动维护 Future 链
  Future<bool> tryDeleteFileByPath(String? path) async {
    if (path.isNullOrEmpty) {
      ZLog.e(_tag, "tryDeleteFileByPath path empty.");
      return false;
    }
    return await tryDeleteFileByFile(File(path!));
  }

  /// 删除排队：手动维护 Future 链，和写操作共用一个队列。
  Future<bool> tryDeleteFileByFile(File? file) async {
    if (file == null) {
      ZLog.e(_tag, "tryDeleteFileByFile file=null");
      return false;
    }
    final path = file.absolutePath!;
    // 1. 获取该路径当前的“最后一道工序”
    ...
    // 2. 创建“我这一棒”的 Completer。Completer用于精准控制future的结束。
    // Completer 用于获取future，并指定其完成时机。。
    final currentTaskCompleter = Completer<bool>();
    final currentTask = currentTaskCompleter.future;
    // 3. 更新 Map，让下一个人等我的 selfCompleter.future
    // 这样下一个人只会等我这一棒跑完，而不会被卷入更长的链条
    _writeQueues[path] = currentTask;
    ...
    previousTask.whenComplete(() async {
      try {
        ...
        ZLog.d(_tag, "_deleteFile success, $path", );
        // 这一行执行后，外部 await writeFile() 的地方会立即恢复执行
        currentTaskCompleter.complete(true);
      } catch (e, trace) {
        ZLog.e(_tag, "", e, trace);
        // complete或者completeError，都会走到whenComplete，then和catchError才是互斥的。
        currentTaskCompleter.complete(false);
      } finally {
        // 移除内容。
        if (_writeQueues[path] == currentTask) {
          _writeQueues.remove(path);
